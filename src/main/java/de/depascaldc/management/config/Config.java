package de.depascaldc.management.config;
	...
	public boolean load(String file, int type, ConfigSection defaultMap) {
		...
		if (!this.file.exists()) {
			try {
				...
				ServerManager.getLogger().error("Could not create Config " + this.file.toString(), e);
			}
			...
			if (this.correct) {
				...
				try {
					...
					ServerManager.getLogger().error(e.getMessage() + e.getStackTrace().toString());
				}
		...
		for (final String line : content.split("\n")) {
			if (Pattern.compile("[a-zA-Z0-9\\-_.]*+=+[^\\r\\n]*").matcher(line).matches()) {
				...
				if (this.config.containsKey(key)) {
					ServerManager.getLogger().debug("[Config] Repeated property " + key + " on file " + this.file.toString());
				}
				switch (valueLower) {
				case "on":
				case "true":
